...
fetchToday(sessionStorage.getItem('currentCity') ? sessionStorage.getItem('currentCity') :'kuala lumpur');
fetchHourly(sessionStorage.getItem('currentCity') ? sessionStorage.getItem('currentCity') :'kuala lumpur');
    ...
    for (let i = 0; i < 5; i++) {
        ...
        if (i === 4) {
            chartCtn.innerHTML += `<div class="chart--item">
                                <div class="chart--temp" style="top: ${50 * (1 - ratio) - 15
                                }%">${Math.round(dataArray[i].temp)}&deg;C</div>
                                <div class="chart--temp last"style="top: ${
                                  50 * (1 - nextRatio) - 15
                                }%">${Math.round(dataArray[i + 1].temp)}&deg;C</div>
                                <div class="chart--line" style="clip-path: polygon(0 ${50 * (1 - ratio)}%, 100% ${50 * (1 - nextRatio)}%, 100% 100%, 0 100%)"></div></div>`;
        } else {
            chartCtn.innerHTML += `<div class="chart--item">
...
function displayDaily(data) {
    ...
    let day = dayArray[new Date(data.daily.time[i]).getDay()].toUpperCase();
        ...
        <div class="daily__day__lohi">
            <span class="deg-low">${Math.round(data.daily.temperature_2m_min[i])}/</span>
            <span class="deg-high">${Math.round(data.daily.temperature_2m_max[i])}°C</span>
        </div>
    </div>`;
    }
}